...
export async function ExploreGrid({ 
  ...
  if (view === "bookmarks") {
    ...
    if (!user) {
      return null; // Already handled by FeaturedGrid
    }
  ...
  // Explore section: skip first 6 (featured), take up to 20 more
  const exploreEvents = sortedEvents.slice(6, 26);

  if (exploreEvents.length === 0) {
    return null;
  }

  return <ExploreSection events={exploreEvents} />;
}
